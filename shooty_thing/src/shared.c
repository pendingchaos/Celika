...
bool gui_button(float posy, const char* label) {
    ...
    draw_add_aabb(aabb, draw_create_rgb(brightness, brightness, brightness));
    draw_set_tex(NULL);
    
    uint32_t* utf32 = utf8_to_utf32((uint8_t*)label);
    
    float text_height = aabb.height * BUTTON_TEXT_SIZE;
    float text_width = font_drawn_width(font, utf32, text_height);
    float text_pos[] = {aabb_cx(aabb)-text_width/2, aabb_cy(aabb)-text_height/2};
    font_draw(font, utf32, text_pos, draw_create_rgb(0, 0, 0), text_height);
